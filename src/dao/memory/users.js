...
class userManager {
  ...
  post = async (user) => {
    if (fs.existsSync(`${this.path}users.json`)) {
      ...
      let objects = await JSON.parse(
        fs.readFileSync(`${this.path}users.json`, "utf-8")
      );
  ...
  getOne = async (search) => {
    if (fs.existsSync(`${this.path}users.json`)) {
      ...
      if (search.name)
        user = users.find((element) => element.name == search.name);
      if (search.category)
        user = users.find((element) => element.category == search.category);
      if (search.price)
        user = users.find((element) => element.price == search.price);
